...
def test_quality_gate_detects_transcript_error_and_accepts_clean_match() -> None:
    ...
    qc.transcribe = lambda *_args, **_kwargs: ("This sentence lost several important words", [], True)  # type: ignore[method-assign]
    failed = qc.evaluate(audio, sr, expected, Path("reference.wav"), [0.54, 0.56])
    assert failed.passed is False
    assert any("ASR mismatch" in reason or "missing words" in reason for reason in failed.reasons)
    assert word_error_rate(expected, failed.transcript) > 0.3
...
def test_production_gate_does_not_ship_known_bad_chunk_after_retries(tmp_path: Path) -> None:
    async def run():
        ...
        assert len(engine.calls) == 3  # first pass + two focused retries
...
def test_v150_colab_keeps_l4_and_verifies_qc_packages() -> None:
    path = ROOT / "colab" / "SoftMeta_Chatterbox_TTS_Colab_v1.5.0.ipynb"
