(() => {
  ...
  function countWords(text) {
    // Emotion control tokens are visible for review but are not spoken words.
  ...
  function captureTab(tab) {
    if (!tab?.panel) return;
    const panel = tab.panel;
    ['title', 'text', 'preset', 'language', 'voice_filename', 'output_format', 'senior_pace_profile', 'cut_start', 'cut_end'].forEach(name => {
      const element = field(panel, name);
  ...
  function renderVoiceQuality(tab, quality) {
    const box = tab?.panel?.querySelector('[data-role="voice-quality"]');
    if (!box) return;
    if (!quality) {
      box.classList.add('hidden');
      box.textContent = '';
    ...
    const warnings = Array.isArray(quality.warnings) && quality.warnings.length
      ? ` • ${quality.warnings[0]}` : '';
    box.classList.remove('hidden');
    box.textContent = `Reference Quality ${quality.score ?? 0}/100 • ${quality.rating || 'Unknown'} • ${quality.duration_seconds ?? '?'} sec${warnings}`;
  }

  async function checkSelectedVoiceQuality(tab) {
    captureTab(tab);
    if (!tab.voice_filename) {
      renderVoiceQuality(tab, null);
      return;
    }
    try {
      const kind = tab.voice_mode === 'predefined' ? 'predefined' : 'clone';
      const quality = await api(`/api/voices/${encodeURIComponent(kind)}/${encodeURIComponent(tab.voice_filename)}/quality`);
      renderVoiceQuality(tab, quality);
    } catch (_) {
      renderVoiceQuality(tab, null);
    }
  }
  ...
  async function uploadVoice(tab, file) {
    ...
    try {
      ...
      saveTabs();
      renderVoiceQuality(tab, result.quality);
  ...
  function wirePanel(tab) {
    ...
    field(panel, 'voice_filename').addEventListener('change', () => checkSelectedVoiceQuality(tab));
    panel.querySelector('[data-action="save-performance"]')?.addEventListener('click', () => savePerformanceFeedback(tab));
  ...
  async function savePerformanceFeedback(tab) {
    if (!tab?.job_id) return;
    const panel = tab.panel;
    const platform = panel.querySelector('[data-role="feedback-platform"]')?.value || 'youtube';
    const introRaw = panel.querySelector('[data-role="feedback-intro"]')?.value;
    const avdRaw = panel.querySelector('[data-role="feedback-avd"]')?.value;
    const status = panel.querySelector('[data-role="feedback-status"]');
    try {
      const result = await api('/api/performance/feedback', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          job_id: tab.job_id,
          platform,
          intro_retention_percent: introRaw === '' ? null : Number(introRaw),
          average_view_duration_seconds: avdRaw === '' ? null : Number(avdRaw),
          views: null,
          notes: '',
        }),
      });
      if (status) status.textContent = `Saved • ${result.samples} comparable sample${result.samples === 1 ? '' : 's'}`;
      toast('Performance feedback saved locally.', 'success');
    } catch (error) {
  ...
  async function showGenerated(tab, job) {
    ...
    const assets = [
      ...
      ['download-vtt', job.vtt_filename, `/api/jobs/${job.id}/asset/vtt`],
    ];
    assets.forEach(([role, filename, href]) => {
      const element = panel.querySelector(`[data-role="${role}"]`);
      if (!element) return;
      element.classList.toggle('hidden', !filename);
      if (filename) { element.href = href; element.download = ''; }
    });
    const q = job.quality_summary;
    const qualityBox = panel.querySelector('[data-role="production-quality"]');
    if (qualityBox && q) {
      qualityBox.classList.remove('hidden');
      const wer = q.average_wer == null ? 'ASR fallback' : `ASR mismatch ${(Number(q.average_wer) * 100).toFixed(1)}%`;
      ...
      qualityBox.textContent = `Production QC • score ${q.average_score ?? '?'} • ${q.checked_chunks} chunks checked • ${q.retries} focused retries • ${wer} • ${speaker}`;
    } else if (qualityBox) {
