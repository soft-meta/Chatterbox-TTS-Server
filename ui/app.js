(() => {
  ...
  function renderVideoEngineStatus(status) {
    ...
    if (status?.ready) {
      ...
      small.textContent = status?.message || 'Run the v0.9.1 A100 40GB installation cells.';
    }
  }
  ...
  function validateVideoPayload(payload) {
    ...
    if (!state.initial?.avatar?.ready) throw new Error('Avatar engine is not installed. Run the v0.9.1 A100 40GB notebook installation cells.');
  }
