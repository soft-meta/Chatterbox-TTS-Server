...
class QualityController:
    ...
    def evaluate(
        ...
        if asr_available and expected_words >= 5:
            ...
            # ASR is a verifier, not an oracle. Senior voices, clone timbre, number
            # expansions and abbreviations can make Whisper produce a 30–40% WER
            # even when the TTS audio is complete. Such cases deserve a focused
            # retry, but must not kill an 8–12 minute job by themselves.
            if wer > 0.30:
            ...
            repetition_loop = _has_repetition_loop(transcript)
            if repetition_loop:
                reasons.append("possible repeated or hallucinated words")
                severe = True

            # Hard rejection requires corroborating evidence. A low ASR word count
            # with normal senior-paced audio is usually a verifier miss; a low count
            # plus very poor ordered coverage / implausibly short speech is much more
            # likely to be a genuinely incomplete generation.
            if ratio < 0.42 and coverage < 0.42:
                reasons.append("strong evidence of missing speech")
                severe = True
            elif ratio < 0.62 and coverage < 0.58 and metrics.get("wpm", 0.0) > 180.0:
                reasons.append("missing speech corroborated by short audio")
                severe = True
            elif wer > 0.72 and coverage < 0.45:
                reasons.append("ASR content verification failed severely")
                severe = True
            elif ratio > 1.65 and wer > 0.60:
                reasons.append("strong evidence of extra speech")
                severe = True
        ...
        return ChunkQualityReport(
            ...
            retry_recommended=retry_recommended or severe,
            hard_failure=severe,
        )
