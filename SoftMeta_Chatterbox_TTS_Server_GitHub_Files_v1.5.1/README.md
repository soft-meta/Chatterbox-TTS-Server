...
## v1.5.1 Production Quality

## v1.5.1 ASR-resilient Production QC

- A 30–40% Whisper/ASR mismatch is now treated as a focused-retry warning instead of an automatic hard failure.
- Hard rejection now requires corroborating evidence such as extreme ordered-word loss, implausibly short speech, repetition/hallucination, clipping, severe silence or speaker drift.
- The queue keeps the best candidate across the original pass and up to two focused retries.
- If the best candidate is acoustically complete but ASR remains uncertain, the long-form job continues and records an accepted-ASR-warning in Production QC.
- Genuine multi-signal content/audio failures still stop the job rather than silently shipping known-bad speech.
...
Open `colab/SoftMeta_Chatterbox_TTS_Colab_v1.5.1.ipynb` and run all cells. The notebook requests a GPU runtime with an L4 preference through Colab metadata. Actual L4 allocation remains controlled by Google Colab availability and account access.
