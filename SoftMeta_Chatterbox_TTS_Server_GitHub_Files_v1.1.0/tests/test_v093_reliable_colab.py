...
def test_optional_installers_are_idempotent() -> None:
    ...
    avatar = (ROOT / "scripts" / "install_echomimic_v3_flash_a100.sh").read_text(encoding="utf-8")
    assert ".softmeta_moss_runtime_v100" in moss
    assert ".softmeta_echomimic_v3_flash_v110" in avatar
    assert "runtime_ok" in avatar
    assert "allow_patterns" in avatar
    assert "echomimicv3-flash-pro/diffusion_pytorch_model.safetensors" in avatar
    assert "FLASH_REVISION" in avatar
    assert "free_gb < 28" in avatar
