...
READY_MARKER="$MODEL_ROOT/.softmeta_longcat_avatar_v15_v100"
...
runtime_ok() {
  ...
  [[ -f "$LONGCAT_DIR/.softmeta_longcat_runtime_patch_v100" ]] || return 1
...
"$MM" run -n "$ENV_NAME" python -m pip install -r "$LONGCAT_DIR/requirements_avatar.txt"
...
required = [
    ...
    repo / ".softmeta_longcat_runtime_patch_v100",
