...
AUTO_ALLOWED_TAGS = {"happy", "sigh", "surprised", "dramatic"}
AUTO_FORBIDDEN_TAGS = {
    "angry", "laugh", "chuckle", "crying", "sarcastic", "fear", "gasp",
...
_THRESHOLDS = {"happy": 2.4, "sigh": 2.8, "surprised": 3.0, "dramatic": 4.0}
_LABELS = {"happy": "warm", "sigh": "reflective", "surprised": "surprised", "dramatic": "serious"}
...
def _score_sentence(text: str, previous: str, following: str) -> tuple[str | None, float]:
    ...
    if first_person and scores["sigh"] > 0:
        scores["sigh"] += 0.45
    if "!" in clean:
...
def analyze_serious_senior_advisor(text: str) -> EmotionAnalysis:
    ...
    if total_words < 120:
        ...
        max_tags = min(10, max(1, total_words // 130))
    min_gap_words = 95
    tag_limits = {
    ...
    for candidate in sorted(candidates, key=lambda item: (-item.score, item.sentence.start_word)):
        ...
        if any(abs(position - chosen.sentence.start_word) < min_gap_words for chosen in selected):
            continue
        # Avoid two auto cues on the same visual/text line even if the line is long.
        if any(candidate.sentence.line_index == chosen.sentence.line_index for chosen in selected):
            continue
