...
def test_auto_emotion_is_sparse_and_non_comedic() -> None:
    ...
    result = analyze_serious_senior_advisor(text)
    # About one cue per 130 words, capped at ten, with additional spacing rules.
    expected_cap = min(10, max(1, result.total_words // 130))
...
def test_backend_and_ui_use_hidden_auto_emotion_pipeline() -> None:
    ...
    assert 'generation_text = emotion_analysis.tagged_text' in queue_source
...
def test_colab_requests_l4_and_final_version_is_125() -> None:
    notebook_path = ROOT / "colab" / "SoftMeta_Chatterbox_TTS_Colab_v1.2.5.ipynb"
    notebook = json.loads(notebook_path.read_text(encoding="utf-8"))
    assert notebook["metadata"]["accelerator"] == "GPU"
    assert notebook["metadata"]["colab"]["gpuType"] == "L4"
    server_source = (ROOT / "server.py").read_text(encoding="utf-8")
    assert 'APP_VERSION = "1.2.5"' in server_source
