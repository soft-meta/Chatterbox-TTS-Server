...
def test_v100_avatar_engine_uses_official_longcat_cli() -> None:
    source = (ROOT / "avatar_engine.py").read_text(encoding="utf-8")
    assert "run_demo_avatar_single_audio_to_video.py" in source
    assert '"--use_distill"' in source
    assert '"--use_int8"' in source
    assert '"--model_type=avatar-v1.5"' in source
