...
def test_colab_is_small_and_has_no_embedded_patch() -> None:
    notebook_path = ROOT / "colab" / "SoftMeta_Chatterbox_TTS_Colab_v1.0.3.ipynb"
...
def test_optional_installers_are_idempotent() -> None:
    ...
    assert ".softmeta_longcat_avatar_v15_v103" in avatar
    assert "libsndfile1" in avatar
    assert "tritonserverclient" in avatar
    assert "requirements_avatar" in avatar
