...
AVATAR_MODEL_REVISION="d3cfdd023b970061e6338c6e5094f7b12e804873"
READY_MARKER="$MODEL_ROOT/.softmeta_longcat_avatar_v15_v101"
...
runtime_ok() {
  ...
  [[ -f "$LONGCAT_DIR/.softmeta_longcat_runtime_patch_v101" ]] || return 1
  ...
  [[ -f "$AVATAR_MODEL_DIR/lora/dmd_lora.safetensors" ]] || return 1
...
# Upstream lists libsndfile1==0.0.1 as though it were a Python package. It is
# an Ubuntu system library (already installed by the Colab setup cell), and no
# matching PyPI distribution exists. Keep every real Avatar dependency while
# excluding only this invalid pip entry.
grep -Ev '^libsndfile1([<=>!~ ]|$)' "$LONGCAT_DIR/requirements_avatar.txt" > "$requirements_avatar"
...
required = [
    ...
    repo / ".softmeta_longcat_runtime_patch_v101",
    ...
    avatar / "lora/dmd_lora.safetensors",
