...
from utils import (
    ...
    prepare_american_english_tts_text,
    safe_filename,
    split_text,
)
...
class QueueManager:
    ...
    async def create(self, request: AudioJobCreate, enqueue: bool = True) -> dict[str, Any]:
        ...
        if (
            ...
            generation_text = prepare_american_english_tts_text(emotion_analysis.tagged_text)
        ...
        try:
            for index, chunk in enumerate(chunks, start=1):
                ...
                if options.get("model") == "chatterbox-turbo":
                    audio = self._compact_excessive_silence(audio, result.sample_rate)
                    ...
                    if (
                        ...
                        retry_audio = self._compact_excessive_silence(retry_audio, retry_result.sample_rate)
