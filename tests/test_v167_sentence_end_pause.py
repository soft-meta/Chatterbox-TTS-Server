...
def test_pause_compactor_uses_selected_target_without_touching_short_pause():
    ...
    out = compact_excessive_sentence_silence(
        audio,
        sr,
        min_silence_ms=420,
        internal_keep_ms=360,
    )
...
def test_queue_manager_reads_sentence_end_pause_option():
    ...
    assert 'internal_keep_ms=sentence_end_pause_ms' in source


def test_version_is_167():
    source = (ROOT / 'server.py').read_text(encoding='utf-8')
    assert 'APP_VERSION = "1.6.7"' in source
...
def test_pause_control_never_lengthens_existing_silence():
    from queue_manager import compact_excessive_sentence_silence
    sr = 24000
    tone = (0.03 * np.sin(2 * np.pi * 180 * np.arange(int(sr * .3)) / sr)).astype(np.float32)
    pause = np.zeros(int(sr * .52), dtype=np.float32)
    audio = np.concatenate([tone, pause, tone])
    out = compact_excessive_sentence_silence(audio, sr, min_silence_ms=480, internal_keep_ms=600)
    assert len(out) <= len(audio)
    assert len(out) >= len(audio) - int(sr * .03)
