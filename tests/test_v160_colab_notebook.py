...
NB=Path(__file__).resolve().parents[1] / 'colab' / 'SoftMeta_Chatterbox_TTS_Colab_v1.6.9.ipynb'


def test_colab_notebook_is_fresh_and_persistent():
    assert NB.exists(), 'v1.6.9 notebook has not been built yet'
    nb=json.loads(NB.read_text())
    joined='\n'.join(''.join(c.get('source',[])) for c in nb['cells'])
    assert 'v1.6.9' in joined
    assert "APP_VERSION == '1.6.9'" not in joined
