...
class AvatarEngineService:
    ...
    def _headless_process_env() -> dict[str, str]:
        """Return a stable environment for Ditto and its headless dependencies.

        Colab exports the notebook-only matplotlib_inline backend.  The isolated
        avatar environment does not contain that IPython backend, while
        MediaPipe imports Matplotlib during Ditto startup.  Always select the
        non-interactive backend for checks, FFmpeg helpers, and inference.
        """
    ...
    def backend_status(self) -> list[dict[str, Any]]:
        ...
        onnxruntime_ok = self._python_can_import("onnxruntime")
        mediapipe_ok = self._python_can_import("mediapipe")
        einops_ok = self._python_can_import("einops")
        ...
        for item in self._backends():
            module_ok = (
                ...
                and mediapipe_ok
                and einops_ok
                and (tensorrt_ok if item.id == "ditto_trt" else True)
            )
            ready = bool(
                python_ok
    ...
    def _resolve_backend(self, requested: str) -> DittoBackend:
        ...
        for backend_id in order:
            ...
            if not item:
                continue
            runtime_ok = onnxruntime_ok and mediapipe_ok and einops_ok and (
                backend_id != "ditto_trt" or self._python_can_import("tensorrt")
            )
            if (
                python_ok
                and runtime_ok
    ...
    def _run_process(
        ...
        with log_path.open("a", encoding="utf-8", errors="replace") as log:
            ...
            self._active_process = subprocess.Popen(
                ...
                env=self._headless_process_env(),
            )
            try:
