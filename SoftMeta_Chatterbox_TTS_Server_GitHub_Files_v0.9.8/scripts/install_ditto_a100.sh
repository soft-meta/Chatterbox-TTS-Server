...
READY_MARKER="$DITTO_DIR/.softmeta_ditto_pytorch_v098"
export PIP_CACHE_DIR="${PIP_CACHE_DIR:-/content/.cache/pip}"
# Colab's notebook backend is not installed inside avatar310. MediaPipe imports
# Matplotlib at module load, so force a headless backend for every installer,
# verification, and Ditto import command.
...
if [[ "$FORCE" != "1" ]] && runtime_ok; then
  # Recover instantly from a previous installer that finished the downloads but
  # failed only during its final import check (for example, Colab MPLBACKEND).
  touch "$READY_MARKER"
